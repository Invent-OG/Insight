...
//       .to(linksRef.current, { opacity: 1, pointerEvents: 'all', stagger: 0.1 }, '-=0.8')
...
const Nav: React.FC<Props> = ({ isLoading }) => {
  ...
  const [isOpen, setIsOpen] = useState(false);
  const [isScrolled, setIsScrolled] = useState(false);
  const [showNav, setShowNav] = useState(true); // ✅ show/hide navbar based on scroll
  ...
  // ✅ Detect scroll direction (hide nav on scroll down, show on scroll up)
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <div ref={wrapperRef}>
      <div
        className={cn(
          'lg:px-28 px-5 py-3 w-full flex items-center  justify-between fixed z-50 transform',
          isScrolled
            ? 'backdrop-blur-md  ' // ✅ frosted blur background
            : 'bg-transparent',
      ...
      >
        ...
        {!isOpen && (
          <div
            ...
            onKeyDown={(e) => e.key === 'Enter' && openMenu()}
          >
            <span className='w-[20px] h-[3px] rounded-sm mb-[3px] transition-all duration-300 bg-primary' />
            <span className='w-[20px] h-[3px] rounded-sm mb-[3px] transition-all duration-300 bg-primary' />
            <span className='w-[20px] h-[3px] rounded-sm mb-[3px] transition-all duration-300 bg-primary' />
          </div>
        )}
