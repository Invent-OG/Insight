    ...
    try {
      if (imageUrl) {
        ...
        if (path) {
          ...
          const { data, error } = await supabase.storage.from('testimonials').remove([path]);
    ...
    <div>
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
        {paginatedTestimonials.map((testimonial) => (
          <Card key={testimonial.id} className='p-6 flex flex-col h-full justify-between'>
            ...
            <p className='text-muted-foreground line-clamp-3 mb-4'>{testimonial.content}</p>
            <div className='flex justify-between items-center'>
              ...
              <div className='flex gap-2'>
                ...
                <DeleteConfirmation
                  onDelete={() => handleDelete(testimonial.id, testimonial.imageUrl!)}
